...
public class SmartExecutor implements Executor {
    ...
    protected synchronized void initThreadPool() {
        Elog.v(TAG, "SmartExecutor core-queue size: " + coreSize + " - " + queueSize
    ...
    public void execute(final Runnable command) {
        ...
        if (callerRun) {
                Elog.i(TAG, "SmartExecutor task running in caller thread");
            command.run();
        }
    }
        ...
        synchronized (lock) {
            ...
            if (waitingList.size() > 0) {
                ...
                if (waitingRun != null) {
                    ...
                    Elog.v(TAG, "Thread " + Thread.currentThread().getName() + " execute next task..");
                } else {
                    Elog.e(TAG,
                            "SmartExecutor get a NULL task from waiting queue: " + Thread.currentThread().getName());
                }
            } else {
                Elog.v(TAG, "SmartExecutor: all tasks is completed. current thread: " +
    ...
    public void printThreadPoolInfo() {
        Elog.i(TAG, "___________________________");
        Elog.i(TAG, "state (shutdown - terminating - terminated): " + threadPool.isShutdown()
                    + " - " + threadPool.isTerminating() + " - " + threadPool.isTerminated());
        Elog.i(TAG, "pool size (core - max): " + threadPool.getCorePoolSize()
                    + " - " + threadPool.getMaximumPoolSize());
        Elog.i(TAG, "task (active - complete - total): " + threadPool.getActiveCount()
                    + " - " + threadPool.getCompletedTaskCount() + " - " + threadPool.getTaskCount());
        Elog.i(TAG, "waitingList size : " + threadPool.getQueue().size() + " , " + threadPool.getQueue());
    }
