...
import com.elt.framework.library.http.data.Charsets;
...
import com.elt.framework.library.http.exception.ServerException;
 ...
 */
public class ApacheHttpClient extends EltHttp {
    ...
    protected <T> void connectWithRetries(AbstractRequest<T> request, InternalResponse response)
        ...
        while (retry) {
            try {
                ...
                if (headers != null) {
                    NameValuePair hs[] = new NameValuePair[headers.length];
                    for (int i = 0; i < headers.length; i++) {
                        ...
                        hs[i] = new NameValuePair(name, value);
                    }
